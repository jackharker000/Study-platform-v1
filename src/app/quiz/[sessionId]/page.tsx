...
export default function QuizPage() {
  ...
  useEffect(() => {
    fetch(`/api/session/${sessionId}`)
      ...
      .then(data => {
        setQuiz({
          sessionId,
          mode: data.session.mode as SessionMode,
  ...
  function goNext() {
    ...
    if (quiz.currentIdx + 1 >= quiz.questions.length) {
      router.push(`/results/${quiz.sessionId}`)
      return
    }
  ...
  useEffect(() => {
    function onKey(e: KeyboardEvent) {
      ...
      if (quiz.mode === 'flashcard') {
        ...
        if (e.key === 'ArrowLeft' && quiz.currentIdx > 0) {
          setQuiz(q => q ? { ...q, currentIdx: q.currentIdx - 1 } : q)
  ...
  if (quiz.mode === 'flashcard' && currentQ) {
    return <FlashcardView quiz={quiz} pending={pending} setPending={setPending} goNext={goNext} />
  }
  ...
  return (
    <div style={{ maxWidth: '860px', margin: '0 auto', padding: '20px 16px 100px' }}>
      ...
      >
        ...
        {/* Feedback (practice/tutor mode after answering) */}
      ...
      <div style={{ display: 'flex', gap: '8px', marginTop: '12px', justifyContent: isAnswered ? 'space-between' : 'flex-end' }}>
        {!isAnswered ? (
          ...
          <>
            <Button variant="ghost" size="sm" onClick={() => router.push(`/results/${quiz.sessionId}`)}>
              End Quiz
            </Button>
...
function FlashcardView({
  quiz, pending, setPending, goNext,
}: {
  quiz: QuizState
        ...
        <Button
          ...
          onClick={() => {
            if (quiz.currentIdx > 0) {
              ;(quiz as unknown as { currentIdx: number }).currentIdx--
            }
          }}
        >
          ← Previous
