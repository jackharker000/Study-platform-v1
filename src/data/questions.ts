...
export const QUESTIONS: Question[] = [
  // ─── AS PHYSICS ──────────────────────────────────────────────────
  {
    ...
    aiMarkable: false, markScheme: undefined,
  },
  {
...
export function getPapersForSubject(subjectId: string): string[] {
  return [...new Set(QUESTIONS.filter(q => q.subject === subjectId).map(q => q.paper))]
}

export function filterQuestions(
  subjectId: string | undefined | null,
  filters: { paper?: string; topic?: string; difficulty?: string; questionType?: string },
): Question[] {
